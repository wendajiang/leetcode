...
public:
    int waysToMakeFair(vector<int>& nums) {
        ...
        for (int i = 0; i < len; i++) {
            evensum[i + 1] = evensum[i] + (!(i & 0x1)) * nums[i];
            oddsum[i + 1] = oddsum[i] + (i & 0x1) * nums[i];
        }
        for (int i = 0; i < len; i++) {
            int tmpoddsum = oddsum[i] + (evensum[len] - evensum[i + 1]);
            int tmpevensum = evensum[i] + (oddsum[len] - oddsum[i + 1]);
            if (tmpoddsum == tmpevensum) {
                cnt ++;
            }
        }
        return cnt;
