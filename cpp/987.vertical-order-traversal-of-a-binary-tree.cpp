...
public:
    vector<vector<int>> verticalTraversal(TreeNode* root) {
        ...
        if (!root) return ans;
        map<int, multiset<pair<int, int>>> path;
        dfs(root, 0, 0, path);
...
private:
    void dfs(TreeNode* node, int row, int col, map<int, multiset<pair<int, int>>> &path) {
        path[col].insert(make_pair(row, node->val));
        if (node->left) dfs(node->left, row + 1, col - 1, path);
        if (node->right) dfs(node->right, row + 1, col + 1, path);
    }
};
// @lc code=end
