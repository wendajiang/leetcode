...
public:
    TreeNode* constructMaximumBinaryTree(vector<int>& nums) {
        TreeNode* root = consructCore(nums, 0, nums.size() - 1);
        return root;
    }

private:
    TreeNode* consructCore(vector<int> &nums, int l, int r) {
        ...
        if (find_index != -1) {
            TreeNode *tmp = new TreeNode(mm);
            tmp->left = consructCore(nums, l, find_index - 1);
            tmp->right = consructCore(nums, find_index + 1, r);
            return tmp;
        } else {
            return nullptr;
        }
